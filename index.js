...
app.post('/poweroutage', async (req, res) => {
  try {
    const { serialno, complaint } = req.body;
      ...
      <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:cmb="http://oracle.com/CmBotPocInterfaceV2.xsd">
        ...
        <soapenv:Body>
            <cmb:CmBotPocInterfaceV2>
                <cmb:request>
                    <cmb:requestSequence>3</cmb:requestSequence>
                    <cmb:billId>?</cmb:billId>
                    <cmb:complaintDetails>${complaint}</cmb:complaintDetails>
    ...
    xmlParser.parseString(soapResponse.data, (err, result) => {
      ...
      const restResponse = {
        message: `${message}\nand your Complaint Id is ${caseId}`
      };
